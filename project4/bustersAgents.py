...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    for counter in livingGhostPositionDistributions:
        mostLikely.append(counter.argMax())
    
    goal = None
    min = self.distancer.getDistance(pacmanPosition, mostLikely[0])
    for loc in mostLikely:
        mostLikelyDist[loc] = self.distancer.getDistance(pacmanPosition, loc)
        if mostLikelyDist[loc] <= min:
            goal = loc
            min = mostLikelyDist[loc]
    #print "goal = ", goal
    
    min = mostLikelyDist[goal]*2
    minA = None
    for a in legal:
        newDist = self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, a), goal)
        #print "distance after ", a, " = ", newDist
        if (newDist <= min):
            min = newDist
            minA = a
    #print "Chose action ", minA, " with new distance ", min
    return minA
