...
class ParticleFilter(InferenceModule):
  ...
  def initializeUniformly(self, gameState):
    ...
    for i in range(0, self.numParticles):
        self.particles.append(random.choice(allPossible))
  ...
  def observe(self, observation, gameState):
    ...
    # Check if ghost has been eaten
    if noisyDistance == None:
        for i in range(0, len(self.numParticles)):
            self.particles[i] = self.getJailPosition
  
    else:
        weightCounter = util.Counter()
        for p in self.particles:
            weightCounter[p] += emissionModel[util.manhattanDistance(p, pacmanPosition)]

        # Check for collapsed weights
        if weightCounter.totalCount() == 0:
            self.initializeUniformly(gameState)

        else:
            weightCounter.normalize()
            for i in range(0, self.numParticles):
                self.particles[i] = util.sample(weightCounter)
  ...
  def elapseTime(self, gameState):
    ...
    oldP = self.particles
    newP = []
    for i in range(0, len(oldP)):
        newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldP[i]))
  ...
  def getBeliefDistribution(self):
    ...
    for loc in self.legalPositions:
        allPossible[loc] = self.particles.count(loc)*1.0/self.numParticles
    #print allPossible
    return allPossible
...
class JointParticleFilter:
  ...
  def initializeParticles(self):
    ...
    for i in range(0, self.numParticles):
        ...
        for i in range(0, self.numGhosts):
            list.append(random.choice(allPossible))
        tup = tuple(list)
        self.particles.append(tup)
  ...
  def observeState(self, gameState):
    ...
    for newParticle in newParticles:
        ...
        weights[tuple(newParticle)] += weight
    ...
    if weights.totalCount() == 0:
        ...
        weights.normalize()
        for i in range(0, self.numParticles):
            self.particles[i] = tuple(util.sample(weights))
