...
def betterEvaluationFunction(currentGameState):
    ...
    score += 50*(1.0/(len(newFoodList)+1))
    if len(MDistances) > 0:
        score += 20*(1.0/(min(MDistances)+0.01))

    score += 10*(1.0/(len(cDistances)+0.01))
    if (len(capsules) > 0):
        score += 6*(1.0/(min(cDistances)+0.01))
    ...
    for i in range(0, len(newGhostStates)):
        ...
        if newScaredTimes[i] > 0:
            ...
            #score += 5*(1.0/(dToGhost[i]+0.01))
        
            if dToGhost[i] == 0:
                score == score**4
        else:
            score -= 30*(1.0/(dToGhost[i]+0.01)**2)
